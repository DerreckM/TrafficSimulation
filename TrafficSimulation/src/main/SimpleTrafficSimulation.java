...
public class SimpleTrafficSimulation {
	...
	private Double buildDoubleForm(String title, String prompt) {
		...
		form.add(title, numberTest);

		UIForm getForm = form
				.toUIForm(prompt);
		String[] result1 = ui.processForm(getForm);
	...
	private void addSUBMENU(int stateNum) {
		...
		m.add("Traffic pattern", new UIMenuAction() {
			public void run() {
				...
				if (newPatternValue == 1) {
					propertyBag.setTrafficPattern(TrafficType.ALTERNATE);
				}
		...
		m.add("Road segment length", new UIMenuAction() {
			public void run() {
				...
				Double newValueMax = buildDoubleForm(titleMax, prompt);
				propertyBag.setRoadLengthMax(newValueMax);

				Double newValueMin = buildDoubleForm(titleMin, prompt);
				propertyBag.setRoadLengthMin(newValueMin);		
			}
		});
		...
		m.add("Traffic light green time", new UIMenuAction() {
			public void run() {
				...
				Double newValueMax = buildDoubleForm(titleMax, prompt);
				propertyBag.setLightGreenTimeMax(newValueMax);

				Double newValueMin = buildDoubleForm(titleMin, prompt);
				propertyBag.setLightYellowTimeMin(newValueMin);		
			}
		});
		m.add("Traffic light yellow time", new UIMenuAction() {
			public void run() {
				...
				Double newValueMax = buildDoubleForm(titleMax, prompt);
				propertyBag.setLightYellowTimeMax(newValueMax);
		...
		menus[stateNum] = m.toUIMenu("Settings Menu");
